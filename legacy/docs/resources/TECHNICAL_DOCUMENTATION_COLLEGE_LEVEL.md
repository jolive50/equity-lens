...
Browser (Next.js frontend)
   ...
   | HTTP requests
   v
FastAPI API (pipelines/realtime/api.py)
   |
   v
LangGraph workflow (pipelines/realtime/langgraph_workflow.py)
   |
   +--> Working agents (historical, sentiment, smart money)
   +--> Legacy agents (prediction, explanation)
   |
   v
Data services (market data, fundamentals, news)
   |
   v
External APIs (Yahoo Finance, Alpha Vantage, Finnhub, NewsAPI)
```
...
Input (ticker)
   |
   v
Validate input -> ensure ticker exists
   |
   v
Collect S&P 500 data -> market prices and fundamentals
   |
   v
Collect news data -> latest articles
   |
   +--> Historical working agent -> trend analysis
   +--> Sentiment working agent -> news analysis
   |
   v
Coordination agent -> synthesize insights and gate on confidence
   |
   +--> Prediction agent (legacy path)
   +--> Sentiment agent (legacy path)
   +--> Smart money agent
   +--> Explanation agent
   |
   v
...
**LangGraph**: Framework for orchestrating multi-agent applications that use large language models.
